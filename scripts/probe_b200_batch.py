...
def _run(gpu: str) -> str:
    ...
    # dynamic=True: ONE trace covers every (B, S) below. A static compile per
    # shape (what the trainer does, one shape per phase) took minutes each and
    # blew the first sweep's hour; memory is what this run sizes.
    cmodel = torch.compile(model, dynamic=True)
    muon_params, adamw_groups = build_param_groups(
        model.named_parameters(),
        weight_decay=tcfg.weight_decay,
        per_head_attn=tcfg.per_head_muon,
        head_dim=cfg.head_dim,
    )
    muon = Muon(
        muon_params,
        lr=getattr(tcfg, "muon_lr", tcfg.peak_lr),
        momentum=getattr(tcfg, "muon_momentum", 0.95),
        nesterov=True,
        ns_steps=tcfg.muon_ns_steps,
        ns_stable_steps=tcfg.muon_ns_stable_steps,
        update_rms=tcfg.muon_update_rms,
        weight_decay=tcfg.weight_decay,
    )
    adamw = torch.optim.AdamW(
        adamw_groups, lr=tcfg.peak_lr, betas=(0.9, 0.95), eps=1e-8
    )  # noqa: E501
    ...
    emit(
        f"[{props.name}] {total_gb:.0f} GB | params {n_params:,} | torch.compile, fp32 params, bf16 autocast, grouped GEMM={getattr(cfg, 'moe_grouped_gemm', None)}, fused CE chunks={CE_CHUNKS}"  # noqa: E501
    )  # noqa: E501
    static_gb = torch.cuda.memory_allocated() / 1e9
    emit(
        f"  static (params+grads-to-be+opt state after 1st step) measured below; params alone {static_gb:.1f} GB"  # noqa: E501
    )  # noqa: E501
        ...
        for ckpt, batches in variants:
            ...
            for B in batches:
                ...
                try:
                    t_c = step(
                        B, S
                    )  # includes compile/trace for this shape  # noqa: E501
...
@app.function(gpu="H100", timeout=3000)
def probe_h100() -> str:
    return _run("H100")
